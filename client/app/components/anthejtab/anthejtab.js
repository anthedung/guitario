...
Array.prototype.max_chars = function () {
  ...
  var len = this.length;
  for (var i = 1; i < len; i++)
    if (this[i].length > max) max = this[i].length;
  return max;
}
...
jtab.setPalette = function (element) {
  ...
  var bgColor = jtab.getStyle(jQuery(element), 'background-color');
  if (!bgColor || (bgColor == 'transparent') || (bgColor == 'rgba(0, 0, 0, 0)')) {
    bgColor = '#fff';
  }
  Raphael.fn.fingering_text_color = bgColor;
}
jtab.render = function (element, notation_text, cha, val, spanOrDiv) {
  spanOrDiv = spanOrDiv || 'span'; //
  var notation = notation_text || jQuery(element).text() || '';
  notation = notation.replace(/\/.*/, '');
  notation = notation.replace(/^./, function (char) {
    return char.toUpperCase();
  });
  if (!cha) cha = notation.split(':')[0];
  if (!val) val = 1;
  var tabtype = jtab.characterize(notation);
  if (tabtype == 0) return;
  var rndID = "builder_" + jtab.element_count++;
  var canvas_holder = jQuery('<span id="' + rndID + '"></span>').css({
  ...
  jQuery(element).addClass('rendered chordcontainer');
...
jtab.trans_back = function (obj) {
  var pObj = $($(obj)[0].parentNode.parentNode);
  var val = parseInt($($(obj)[0].parentNode.parentNode).children('#trans_code').val()) - 1;
  if (val == 0) val = 10;
  var cha = $($(obj)[0].parentNode.parentNode).children('#trans_char').val();
  jtab.render(pObj, cha + ":" + val, cha, val);
}
jtab.trans_next = function (obj) {
  var pObj = $($(obj)[0].parentNode.parentNode);
  var val = parseInt($($(obj)[0].parentNode.parentNode).children('#trans_code').val()) + 1;
  if (val == 11) val = 1;
  var cha = $($(obj)[0].parentNode.parentNode).children('#trans_char').val();
...
jtab.trans_default = function (obj) {
  var pObj = $($(obj)[0].parentNode.parentNode);
  var cha = $($(obj)[0].parentNode.parentNode).children('#trans_char').val();
  jtab.render(pObj, cha + ":0");
}
jtab.transposeChord = function (chord, amount) {
  ...
  var returnVal = chord.replace(/[CDEFGAB]#?/g, function (match) {
...
jtab.simplify = function (chord) {
  chord = chord.replace(/^./, function (char) {
    return char.toUpperCase();
  });
  chord = chord.replace(/\db5$/g, "");
  chord = chord.replace(/\db9$/g, "");
  chord = chord.replace(/\d#5$/g, "");
  chord = chord.replace(/maj/g, "");
  chord = chord.replace(/dim/g, "");
  chord = chord.replace(/aug/g, "");
  chord = chord.replace(/add/g, "");
...
function changeAllChord(amount) {
  jQuery('.anthe-content span.guitarios_chord').each(function () {
    var obj = jQuery(this);
...
function refreshUsedChords() {
  jQuery('.guitarios_chord_drawing').each(function () {
    var obj = jQuery(this);
    jtab.render(obj, obj.attr("value"));
  });

  jtab.render(jQuery('#chord_view #draw_chord'), jQuery('#chord_view #draw_chord').attr('value'));
}
...
function prepareChordView(){
  ...
  jQuery('body').on('mouseover', 'span.guitarios_chord', function () {
    var obj = jQuery(this);
    clearTimeout(chord_view_delay);
    chord_view_delay = setTimeout(function () {
      var pos = obj.offset();
      jQuery('#draw_chord').html('<p>Loading...</p>');
      jQuery('#chord_view').css({
        position: "absolute",
        top: (pos.top - 180) + "px",
        left: (pos.left - 70) + "px"
      }).slideDown(function () {
        jQuery('#chord_view #draw_chord').attr('value', obj.html());
        jtab.render(jQuery('#chord_view #draw_chord'), obj.html());
      });
    }, 600);
  }).on('mouseleave', 'span.guitarios_chord', function () {
    clearTimeout(chord_view_delay);
  });

  //if (isPhone()) {
  //  $('#song_text:not(span)').hammer().on('touch', function (e) {
  //    $('#chord_view').slideUp();
  //  });
  //} else {
